...
def fetch_menu() -> list:
    sh = _sheets_client().open_by_key(SHEETS_ID)
    rows = sh.worksheet("menu").get_all_records()
    return [r for r in rows if str(r.get("active", "1")) == "1"
            and int(r.get("portions", 0)) > 0]

def fetch_last_order(user_id: int) -> dict | None:
    """Возвращает последний план клиента из таблицы orders, или None."""
    try:
        sh   = _sheets_client().open_by_key(SHEETS_ID)
        ws   = sh.worksheet("orders")
        rows = ws.get_all_records()
        uid  = str(user_id)
        # Ищем последний заказ этого пользователя
        user_rows = [r for r in rows if str(r.get("user_id")) == uid and r.get("plan_json")]
        if not user_rows:
            return None
        last = user_rows[-1]
        return json.loads(last["plan_json"])
    except Exception as e:
        logger.error(f"fetch_last_order error: {e}")
        return None
...
def decrement_inventory(dish_id_counts: dict):
    """dish_id_counts = {"3": 2, "7": 1} — id блюда → сколько порций вычесть"""
    sh = _sheets_client().open_by_key(SHEETS_ID)
    ws = sh.worksheet("menu")
    rows = ws.get_all_records()
    port_col = ws.find("portions").col
    for i, row in enumerate(rows, start=2):
        did = str(row["id"])
        if did in dish_id_counts:
            new_val = max(0, int(row["portions"]) - dish_id_counts[did])
            ws.update_cell(i, port_col, new_val)
...
def save_order(user, summary_text: str, total: float, address: str, plan: dict = None):
    ...
    try:
        ...
        ws = sh.add_worksheet("orders", rows=500, cols=9)
        ws.append_row(["Дата","user_id","Username","Имя","Заказ","Итого","Адрес","Статус","plan_json"])
    ws.append_row([
