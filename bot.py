...
def fetch_menu() -> list:
    ...
    for row in rows[1:]:
        ...
        if str(d.get("active","1")) == "1" and int(d.get("portions",0) or 0) > 0:
            result.append({
                "id":    str(d.get("id","")),
                "name":  str(d.get("name","")),
                "cat":   str(d.get("cat","")),
                "kcal":  int(d.get("kcal",0) or 0),
                "p":     int(d.get("p",0) or 0),
                "f":     int(d.get("f",0) or 0),
                "c":     int(d.get("c",0) or 0),
                "price": float(d.get("price",0) or 0),
                "fresh": str(d.get("fresh","0")) == "1",
            })
    return result
